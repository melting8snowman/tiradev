    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            pygame.draw.rect(screen, settings.black,
                             (0, 0, settings.width, settings.square_size))
            position = event.pos[0]
            if board.turn == 1:
                pygame.draw.circle(screen, settings.red, (position, int(
                    settings.square_size/2)), settings.radius)
            elif board.turn == 2:
                pygame.draw.circle(screen, settings.white, (position, int(
                    settings.square_size/2)), settings.radius)
            else:
                pygame.draw.circle(screen, settings.yellow, (position, int(
                    settings.square_size/2)), settings.radius)
        pygame.display.update()
                ...
                if board.is_valid_location(col):
                    ...
                    if board.is_winning_move(1):
                        label = gamefont.render(
                            "Player One wins!!!!!", 1, settings.red)
                        screen.blit(label, (40, 10))
            ...
            # Player 2 (Human)
                ...
                if board.is_valid_location(col):
                    ...
                    if board.is_winning_move(2):
                        label = gamefont.render(
                            "Player Two wins!!!!", 1, settings.white)
                        screen.blit(label, (40, 10))
